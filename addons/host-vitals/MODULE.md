...
| Version | Notes |
| ------- | ----- |
| 0.0.6-beta.4 | **Made the tile look like part of JonDash rather than a stretched table.** The usage indicator had become a translucent block behind the text, which read as a stray selection highlight rather than a measurement; it is now a proper track-and-fill meter along the bottom edge of each row, still costing no height so short tiles stay correct. A healthy meter is drawn in the app accent instead of the muted grey — `TONE.ok` is right for calm *text* and wrong for a *bar*, where it made a 64% disk look like a rendering fault. Memory now reads as a percentage like every other metered row, with the byte figures following in muted text once there is width for them. Type scales with the tile so a large card is not 12px labels stranded in 117px rows, floored at 11px so a short tile cannot shrink it to 3px. |
| 0.0.6-beta.3 | **Fills the card instead of huddling in its top-left corner.** The previous beta was measured only for overflow, and an empty card overflows nothing — so a large tile showed a few rows across the top and left most of itself blank. Two mechanisms were wrong before this one: flex `flex-wrap` ran columns off the side of the card, and CSS `columns` fixed that but *balances*, spreading a handful of rows one-per-column across the top. It is now a grid whose rows are `1fr`, so they stretch to use the height, flowing into another column only once the height is spent. Measured fill went from about 10% to 86–96% of the card on large tiles, with nothing cut at any normal size. |
