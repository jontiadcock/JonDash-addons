...
// These reach `admin.*`, which needs an ADMIN in ctx.user AND `host-services:configure`. Only
// rendered on the settings panel, but the helper re-checks rather than trusting where the call
// came from — a panel is just a page, and pages can be reached in ways their author did not plan.
...
export const addEntryAction = moduleAction(MODULE_ID, async (ctx, form: FormData): Promise<void> => {
  ...
  if (r.ok) {
    // The risk warning is surfaced at the moment of success rather than buried — "you have
    // just allowed something that can lock you out" is only useful immediately.
    const risk = r.risk.level === "none" ? "" : ` ${r.risk.message}`;
    await notice(ctx, r.risk.level === "lockout" ? "warn" : "ok", `Added ${r.entry.name}.${risk}`);
  } else {
