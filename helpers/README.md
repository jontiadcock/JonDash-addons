...
- **One version, always current, never breaks its API.** No resolution, no conflicts, no version
  ranges. Both sides of the contract are controlled by the same project, so this is a promise we can
  simply keep rather than a problem to solve.
...
| Helper | What it gives a module | Spec |
| ------ | ---------------------- | ---- |
| `scheduler` | Periodic work that runs from server start, declared rather than started | [scheduler/HELPER.md](scheduler/HELPER.md) |

A filesystem helper is agreed in principle and not yet specified. Its shape should be driven by what
its first real consumer needs, decided before the API is designed — otherwise the test proves only
that the helper matches itself.
